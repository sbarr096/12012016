...
function showUsageTable()
{
   ...
   $result = mysql_query("SELECT * FROM radio2 WHERE Time >= CURRENT_TIMESTAMP - INTERVAL 50000 MINUTE ORDER BY Time DESC") or die (mysql_error());
...
{
   connect_db(); 
   $sql="SELECT * FROM radio2  WHERE Time BETWEEN '${from}' AND '${to}' ORDER BY Time ASC";
...
function getNow()
{
   ...
   $result = mysql_query("SELECT * FROM radio2 ORDER BY Time DESC LIMIT 1") or die (mysql_error());
...
{
   connect_db();
   $result = "INSERT INTO radio2 (Watts) VALUES (".$val.")";
